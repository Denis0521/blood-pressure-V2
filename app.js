...
// [新增] 檢查是否有未保存的變更，並根據使用者選擇執行回調
function checkUnsavedChanges(onConfirm, onCancel) {
    if (isDataDirty) {
        if (confirm("⚠️ 您有尚未儲存的修改！\n\n繼續此操作將會遺失未儲存的資料。\n\n確定要繼續嗎？（點擊「取消」可留在本頁繼續編輯）")) {
            isDataDirty = false; // 確認放棄，重置狀態
            if (onConfirm) onConfirm();
        } else {
            if (onCancel) onCancel(); // 取消操作
        }
    } else {
        if (onConfirm) onConfirm(); // 沒有未保存資料，直接執行
    }
}
...
window.addEventListener('load', () => {
    ...
    seatNumberInput.value = ctrlSeat.value;
    
    ctrlSeat.addEventListener('change', function() {
        const newSeat = this.value;
        const oldSeat = seatNumberInput.value;
        // [修改] 切換座號時，檢查是否有未保存的修改
        checkUnsavedChanges(
            () => { seatNumberInput.value = newSeat; }, // 確認切換
            () => { this.value = oldSeat; }             // 取消切換，恢復原座號
        );
    });
    ...
    // [新增] 監聽所有輸入框與文字區域的變化，標記為已修改
    document.querySelectorAll('input, textarea, select').forEach(el => {
        if (el.id === 'ctrlSeat') return; // 座號下拉選單已在上方獨立處理
    ...
    // [新增] 監聽 Checkbox 的變化
    for(let i = 1; i <= 6; i++) {
        $('cb' + i).addEventListener('change', markAsDirty);
    }
});
...
// ==================== 清除表單 ====================
function clearForm() {
    const hasDirty = isDataDirty;
    
    // [修改] 清除畫面前，檢查是否有未保存的修改
    checkUnsavedChanges(() => {
        // 如果原本沒有修改過，還是保留原本的「確定要清除嗎」提示
        if (!hasDirty) {
